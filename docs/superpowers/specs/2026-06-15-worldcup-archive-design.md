...
- **MVP = Phase 0 + Phase 1 (2002만).** 품질 기준을 2002로 확정한 뒤 같은 파이프라인을 5개 대회로 복제.
...
- 채팅: **자체 채팅(우리 ws 서버)**. 치지직 채팅 릴레이는 공식 API 불가(본인 채널만), 비공식 경로는 ToS 위반·차단 위험 → 미채택.
...
- **결정**: 정적 SSG(DB 없음) / 개인서버 배포 / 자체채팅+딥링크 / 2002 수직슬라이스 먼저 / 디자인 시안 6.
